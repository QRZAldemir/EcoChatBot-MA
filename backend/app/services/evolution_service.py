...
async def enviar_texto(instance: str, number: str, text: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            # Adicionado "delay": 1200 para simular digitação humana (Evolution API v2)
            r = await client.post(url, headers=_headers(), json={"number": number, "text": text, "delay": 1200})
            r.raise_for_status()
            return r.json()
...
async def enviar_lista(
    ...
    payload = {
        ...
        "sections": sections,
        "delay": 1200  # Adicionado para simular digitação humana (Evolution API v2)
    }
